...
const SOUNDS = [
  {
    ...
    url: 'https://upload.wikimedia.org/wikipedia/commons/transcoded/d/d6/Rain_storm_-_thunderstorm_with_moderate_rain.ogg/Rain_storm_-_thunderstorm_with_moderate_rain.ogg.mp3',
  },
  {
    id: 'ocean',
    label: 'Ocean',
    icon: <OceanIcon size={20} iconSize={12} />,
    url: 'https://upload.wikimedia.org/wikipedia/commons/transcoded/a/a0/Waves_crashing_on_rocky_shore.ogg/Waves_crashing_on_rocky_shore.ogg.mp3',
  },
  {
    id: 'nature',
    label: 'Nature',
    icon: <NatureIcon size={20} iconSize={12} />,
    url: 'https://upload.wikimedia.org/wikipedia/commons/transcoded/5/5e/Birdsong_1.ogg/Birdsong_1.ogg.mp3',
  },
]
...
export default function MusicPlayer() {
  ...
  // Tracks whether we want playback to start when the src changes
  ...
  useEffect(() => {
    ...
    // Only auto-play if the user selected a track (shouldPlayRef is set)
  ...
  const selectTrack = (id) => {
    ...
    // New track: always start playing
    shouldPlayRef.current = true
    setCurrentId(id)
    setIsPlaying(true)
  }
    ...
    <div style={{ position: 'fixed', top: 12, right: 12, zIndex: 999999, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
      ...
      {isOpen && (
        <div style={{
          ...
          <div style={{ fontSize: 9, fontFamily: 'JetBrains Mono', color: 'var(--text-muted)', padding: '4px 8px 6px', borderBottom: '1px solid var(--border)', marginBottom: 4, textTransform: 'uppercase' }}>
            Ambient Sounds
          </div>

          {SOUNDS.map(s => {
            const active = currentId === s.id && isPlaying
            return (
              <button
                key={s.id}
              ...
              >
                <div style={{ opacity: active ? 1 : 0.6 }}>{s.icon}</div>
                <span style={{ fontSize: 12, fontWeight: active ? 700 : 400 }}>{s.label}</span>
                {active && (
                  <div style={{ marginLeft: 'auto', width: 6, height: 6, borderRadius: '50%', background: 'var(--accent)', boxShadow: '0 0 8px var(--accent)' }} />
                )}
              </button>
            )
          })}
          ...
          <div style={{ borderTop: '1px solid var(--border)', marginTop: 4, paddingTop: 4 }}>
            <button
              ...
              style={{
                ...
                border: 'none', color: isPlaying ? '#EF4444' : '#22C55E',
